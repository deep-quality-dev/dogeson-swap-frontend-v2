...
function useSwapCallArguments(
  ...
  return useMemo(() => {
    if (!trade || !recipient || !library || !routerType || !account || !chainId || !deadline) return []

    const contract: Contract | null = getRouterContract(routerType, chainId, library, account)
    if (!contract) {
      return []
    }
